...
static LogicalResult verifyRegResetOp(RegResetOp op) {
  // If the result is a ground type, the async reset value must be a constant
  // literal or invalid value.
  if (op.getType().isGround() &&
      op.resetSignal().getType().isa<AsyncResetType>() &&
      !isa_and_nonnull<ConstantOp, InvalidValueOp>(
          op.resetValue().getDefiningOp())) {
    return op.emitError(
