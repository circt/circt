// RUN: circt-opt --pass-pipeline='firrtl.circuit(firrtl-infer-resets)' --split-input-file %s | FileCheck %s
...
firrtl.module @NestedAggregates(out %buzz: !firrtl.bundle<foo flip: vector<bundle<a: asyncreset, c: uint<1>, b flip: reset>, 2>, bar: vector<bundle<a: reset, b flip: asyncreset, c: uint<8>>, 2>>) {
  %0 = firrtl.subfield %buzz("bar") : (!firrtl.bundle<foo flip: vector<bundle<a: asyncreset, c: uint<1>, b flip: reset>, 2>, bar: vector<bundle<a: reset, b flip: asyncreset, c: uint<8>>, 2>>) -> !firrtl.vector<bundle<a: reset, b flip: asyncreset, c: uint<8>>, 2>
  %1 = firrtl.subfield %buzz("foo") : (!firrtl.bundle<foo flip: vector<bundle<a: asyncreset, c: uint<1>, b flip: reset>, 2>, bar: vector<bundle<a: reset, b flip: asyncreset, c: uint<8>>, 2>>) -> !firrtl.vector<bundle<a: asyncreset, c: uint<1>, b flip: reset>, 2>
...
firrtl.circuit "Top" {
  ...
  firrtl.module @Top(in %clock: !firrtl.clock, in %reset: !firrtl.asyncreset, in %init: !firrtl.uint<1>, in %in: !firrtl.uint<8>, in %extraReset: !firrtl.asyncreset {firrtl.annotations = [{class = "firrtl.transforms.DontTouchAnnotation"}, {class = "sifive.enterprise.firrtl.FullAsyncResetAnnotation"}]}) {
    ...
    // CHECK: %3 = firrtl.subfield %reset4("a")
    // CHECK: %4 = firrtl.subfield %reg4("a")
    ...
    %0 = firrtl.subfield %reg4("a") : (!firrtl.bundle<a: uint<8>>) -> !firrtl.uint<8>
    ...
    // subfields behind.
    // CHECK-NOT: %16 = firrtl.subfield %reset4("a")
    // CHECK: %16 = firrtl.subfield %reg4("a")
    %3 = firrtl.subfield %reg4("a") : (!firrtl.bundle<a: uint<8>>) -> !firrtl.uint<8>
  }
}
